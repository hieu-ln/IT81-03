...
//DSLK su dung cho STACK va DANH SACH KE
struct Node {
	int info;
	Node* link;
};

Node* First[MAX]; //DS cac LK
...
void inputGraph()
{
	...
	{
		if (i > 0)
			cout << "Nhap DSK cua dih thu " << i - 1 << " (" << vertex[i - 1] << ") : ";
		int u;
		string str;
...
int main()
{
	...
	do {
		...
		case 1:
			n = 10;
			initGraph(n);
			cout << "Ban vau khoi tao DANH SACH KE rong thanh cong\n";
			break;
		case 2:
			inputGraphFromText();
